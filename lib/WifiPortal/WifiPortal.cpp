...
String WifiPortal::criar_div_wifi(const char* nome_wifi, unsigned char potencia_porcentagem, const char* potencia_simbolo, unsigned char id, signed char rssi) {
...
byte WifiPortal::scan_wifis() {    
    ...
    int8_t numero_redes = WiFi.scanComplete();
    log(numero_redes);
    if (numero_redes == -1 or numero_redes == -2) {
        log(F("scanner nao iniciado ou em execucao"));
        return 0;
    }
    return numero_redes;
}

// Função para converter RSSI em qualidade do sinal
inline unsigned char  WifiPortal::get_quality_signal(char rssi) {
    if (rssi >= -50) {
        return 100;  // Excelente
    } else if (rssi <= -100) {
        return 0;    // Muito fraco
    } else {
        // Mapeia de -100 a -50 dBm para 0 a 100%
        return 2 * (rssi + 100);
    }
}
...
void WifiPortal::montar_captive_wifi() {
    String divs_wifis = "";
    byte num_redes = this->scan_wifis();

    if(num_redes == 0){
        divs_wifis = this->criar_div_wifi("Sem Sinal disponivel", 0, "1",1,0);
    }else{
        std::set<String> redes_unicas; // Conjunto para armazenar SSIDs únicos
        byte num_redes_processadas = 0;
        for (byte id_rede = 0; id_rede < num_redes && num_redes_processadas < 10; id_rede++) {
            String ssid = WiFi.SSID(id_rede); // Armazena temporariamente o SSID
            // Verifica se o SSID já foi processado
            if (redes_unicas.find(ssid) != redes_unicas.end()) {
                continue; // Se já existe, pula para a próxima rede
            }

            // Adiciona o SSID ao conjunto de redes únicas
            redes_unicas.insert(ssid);
            yield();
            int rssi = WiFi.RSSI(id_rede);
            log(rssi);
            if (rssi > RSSI_minimo_sinal) {
                log("Rede encontrada: " + ssid); // Usa o valor armazenado
                unsigned char porcentagem_sinal = this->get_quality_signal(rssi);   
                divs_wifis += this->criar_div_wifi(ssid.c_str(), porcentagem_sinal, String(map(porcentagem_sinal, 0, 100, 1, 4)).c_str(),id_rede,rssi);
                num_redes_processadas++;
            }
        }
    }
...
void WifiPortal::set_config_callback(void (*name_callback)(void), unsigned short ms_interval_call) {
